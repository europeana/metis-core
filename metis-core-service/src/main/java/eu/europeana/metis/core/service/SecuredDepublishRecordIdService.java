 ...
 */
@Service
public class SecuredDepublishRecordIdService {
   ...
   * Constructor.
   *
   * @param securedOrchestratorService The orchestrator service
   ...
   */
  @Autowired
  public SecuredDepublishRecordIdService(SecuredOrchestratorService securedOrchestratorService,
      DepublishRecordIdDao depublishRecordIdDao, DatasetDao datasetDao) {
   ...
   * the depublication is for an entire dataset or for individual records ids. Those ids are either provided or all of the ids,
   * previously populated, from the database will be used. Only ids in
   * {@link eu.europeana.metis.core.dataset.DepublishRecordId.DepublicationStatus#PENDING_DEPUBLICATION} will be attempted for
   * depublication.</p>
   *
   * @param datasetId The ID of the dataset for which to retrieve the records. Cannot be null.
  ...
  public boolean canTriggerDepublication(String datasetId)
    ...
    if (securedOrchestratorService.getRunningOrInQueueExecution(datasetId) != null) {
      ...
      final DatasetExecutionInformation executionInformation = securedOrchestratorService
